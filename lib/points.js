...
export async function getLeaderboard(limit = 50) {
  ...
  // Fetch points events AND all fans who have set a username
  ...
  if (pointsRes.error) {
    console.error('getLeaderboard error:', pointsRes.error)
    return []
  }
  ...
  // Include fans who set a username but have no points yet
  for (const p of (profilesRes.data ?? [])) {
    if (!map[p.id]) {
      map[p.id] = { user_id: p.id, email: p.email ?? '', username: p.username, city: p.city ?? null, total: 0 }
    }
  }

  // Only surface fans with a username OR fans with points
